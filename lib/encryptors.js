...
// Reverse Cipher
const reverseCipher = (sentence) => sentence.split(' ').map(word => word.split('').reverse().join('')).join(' ');

// Vigenère Cipher
const vigenereCipher = (str, key, decrypt = false) => {
  if (!key || !key.trim() || !key.match(/^[a-zA-Z]+$/)) {
    throw new Error('Invalid key! Key must contain only letters and cannot be empty.');
  }
  let output = '';
  for (let i = 0, j = 0; i < str.length; i++) {
    ...
    if (char.match(/[a-z]/i)) { // Check if the character is a letter
      ...
      if (decrypt) shift = 26 - shift; // Reverse shift for decryption
      if (code >= 65 && code <= 90) { // Uppercase letters
...
const sha256Hash = (str) => {
  ...
  return crypto.createHash('sha256').update(str, 'utf-8').digest('hex'); // Hash the string using SHA-256
};
