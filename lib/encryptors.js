...
const { Buffer } = require('buffer'); // Import Buffer for Base64
...
const keyStretchHash = (str, { key } = {}) => {
  ...
  const combined = crypto.createHash('sha256').update(stretchedKey + str).digest('hex'); // Combine stretched key with message
  return combined;
};
...
const matrixMixer = (str, { key } = {}) => {
  ...
  const paddedStr = str.padEnd(Math.ceil(str.length / 3) * 3, '\0'); // Pad string to multiple of 3
  let output = '';
...
const doubleHashChain = (str, { key } = {}) => {
  ...
  const firstHash = crypto.createHash('sha256').update(key + str).digest('hex'); // First SHA256 hash with key
  const secondHash = crypto.createHash('sha256').update(firstHash + key).digest('hex'); // Second SHA256 hash with key
  return secondHash;
};
